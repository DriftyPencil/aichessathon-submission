...
def get_move(fen: str, time_left_ms: int) -> str:
    ...
    # Depth 2 is cheap and already sees whether a capture can simply be recaptured.
...
def evaluate(board: chess.Board) -> int:
    """Simple material and activity score for the side to move."""
    side = board.turn
    score = 0
...
def positional_bonus(piece: chess.Piece, square: chess.Square) -> int:
    """Small development hints without a large piece-square table."""
...
def move_priority(board: chess.Board, move: chess.Move) -> int:
    ...
    if board.is_capture(move):
        victim_value = captured_value(board, move)
